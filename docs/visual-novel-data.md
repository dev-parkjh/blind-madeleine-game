...
- `portraits`: object mapping portrait states to asset paths.
  ...
  - `live2d_model`: optional `res://` path to the web rig JSON created by `tools/live2d-editor`. Runtime portrait rendering still uses `path`; the rig path is editor/source metadata for regenerating the portrait.
  - `live2d_motion_frame`: optional metadata for static frames exported from a web-rig motion clip. New exports include semantic `pose_tags`, `pose_score`, and sampled `parameter_values` so the editor/runtime can choose frames by rigged expression intent without parsing the full rig JSON.
  - `live2d_expression_preset`: optional metadata for portraits rendered from saved expression presets. It records the preset id/label, auto-preset flags, semantic `pose_tags` / `pose_score`, and sampled `parameter_values`; preset exports also expose a synthetic `live2d_motion_frame` in the `expression_presets` clip so they can participate in the same adaptive pose flow as motion-frame exports.
  - Adaptive motion selection can be guided per dialogue node with `stage_cast.{character_id}.live2d_motion_clip`, `live2d_motion_time` seconds, `live2d_motion_progress` from `0` to `1`, or `live2d_pose_hint` such as `happy`, `sad`, `angry`, `surprised`, `curious`, `talk`, `blink`, `motion`, `look_left`, `look_right`, `look_up`, `look_down`, `tilt_left`, `tilt_right`, `viseme_a`, `viseme_i`, `viseme_o`, `viseme_u`, or `viseme_closed`. The same keys can be placed in node `metadata` to apply to the current speaker when the cast entry omits them.
  - Set `stage_cast.{character_id}.live2d_motion_loop` to `true` to loop exported frames from the selected clip while the character remains visible. `live2d_motion_speed` controls playback speed from `0.1` to `4`. When `live2d_motion_blend_duration` is omitted, the runtime adjusts frame crossfade length from sampled pose distance so small parameter changes stay quick and larger expression changes soften; setting it explicitly fixes the crossfade duration from `0` to `1` second.
  - Set `stage_cast.{character_id}.live2d_dialogue_motion` to `true` to switch exported motion clips automatically: `talk_loop` while the current speaker's text is typing, then `idle_loop` after the line finishes. Override the default clip ids with `live2d_talk_motion_clip`, `live2d_idle_motion_clip`, and `live2d_viseme_motion_clip`.
- `metadata`: object for game-specific extension data.
  - `live2d_web_model`: optional metadata written by `tools/live2d-editor`. It mirrors generated portrait/model paths under `portraits`, including `motion_frame` and `expression_preset` summaries when present, records the last `source_model_path`, and stores lightweight rig source counts (`portrait_count`, `image_part_count`, `deformer_group_count`, `deformer_parent_count`, `warp_deformer_count`, `visibility_gate_count`, `locked_part_count`, `hit_area_count`, `parameter_count`, `rig_binding_count`, `expression_preset_count`, `auto_expression_preset_count`, `motion_clip_count`, `motion_frame_set_count`), `clips[]` summaries, `expression_presets[]` summaries, exported `motion_frame_sets[]`, `hit_areas[]` summaries, `parameter_bindings[]` summaries, and `adaptive_clip_id` for editor inspection, regeneration, and runtime interaction workflows.
...
```json
{
  "portraits": {
    "idle_loop_f01": {
      ...
      "center": [0.5, 0.18],
      "live2d_model": "res://assets/characters/235db733-cbb2-4c89-86fc-377149f9de48/idle_loop_f01.live2d-web.json",
...
The same export also groups frame portraits by clip under `metadata.live2d_web_model.motion_frame_sets[]`; states rendered from expression presets may include the same `expression_preset` summary mirrored on `metadata.live2d_web_model.portraits.{state}`:
          ...
          "states": [
            {
              ...
              "model_path": "res://assets/characters/235db733-cbb2-4c89-86fc-377149f9de48/idle_loop_f01.live2d-web.json",
              "center": [0.5, 0.18],
...
When a dialogue node shows a speaker but does not explicitly set that speaker's `stage_cast.{character_id}.portrait`, `scripts/visual_novel/web_rig_runtime.gd` cycles through that character's `live2d_motion_frame` portraits to create a natural pose change on dialogue advance. Preset portraits exported with `live2d_expression_preset` are included because they also carry a synthetic `expression_presets` motion frame with semantic pose metadata, and the active game portrait state keeps that `live2d_expression_preset` summary after automatic frame swaps. The runtime also reads `metadata.live2d_web_model.motion_frame_sets[]` and merges those indexed states with the portrait scan, so newly exported clip sets keep their declared frame order and fallback paths. If no clip is explicitly requested, the editor preview and runtime prefer `adaptive_pose`, then `dialogue_pose`, idle, breath, and talk clips. New exports include `clip_duration`, `frame_count`, `physics_sampled`, semantic `pose_tags`, `pose_score`, and sampled `parameter_values`; the runtime uses `clip_duration` for progress targeting and motion loops when available, uses `parameter_values` to prefer a tagged frame that differs from the currently visible pose, and records a recent pose signature from `live2d_expression_preset`, sampled parameters, or pose tags so key-renamed duplicate frames are not treated as fresh poses. It then falls back to the older frame-time estimate. Set `stage_cast.{character_id}.adaptive_live2d_pose` or node `metadata.adaptive_live2d_pose` to `false` to disable the automatic pose pick for that node.

To target a specific exported motion frame, set `stage_cast.{character_id}.live2d_motion_clip` and either `live2d_motion_time` in seconds or `live2d_motion_progress` as a normalized `0` to `1` value. The runtime picks the nearest exported frame in that clip. If time/progress is omitted, set `live2d_pose_hint`, `live2d_pose_tag`, or `live2d_pose_tags` to pick the best matching semantic frame; accepted hints expand through aliases such as `happy` -> `smile` / `laugh`, `talk` -> `open_mouth`, `blink` -> `squint`, `motion` -> look/tilt tags, and `viseme_a` / `viseme_i` / `viseme_o` / `viseme_u` / `viseme_closed` -> exported phoneme mouth poses, matching the web rig editor's `Best Pose` preview. When those are omitted, the runtime infers hints from simple dialogue text cues such as `?`, `!`, ellipses, laughter, apology, anger, or surprise words. Add `live2d_motion_loop: true` to continuously cycle through that clip's exported frames on the active portrait; frame swaps use an automatic pose-distance crossfade by default so subtle loop frames stay snappy while larger expression jumps read more softly. Use `live2d_motion_speed` to slow down or speed up playback, and set `live2d_motion_blend_duration` from `0` to `1` second to force or disable a fixed crossfade. Add `live2d_dialogue_motion: true` to make the active speaker use `talk_loop` while typewriter text is appearing and `idle_loop` afterward; `live2d_talk_motion_clip`, `live2d_idle_motion_clip`, and `live2d_viseme_motion_clip` override those default clip ids. Aliases `motion_clip`, `clip_id`, `motion_time`, `pose_time`, `motion_progress`, `pose_progress`, `pose_hint`, `pose_tag`, `pose_tags`, `emotion`, `mood`, `tone`, `expression`, `live2d_motion_play`, `live2d_motion_autoplay`, `motion_loop`, `motion_play`, `motion_speed`, `motion_blend_duration`, `pose_blend_duration`, `live2d_auto_dialogue_motion`, `dialogue_motion`, `auto_dialogue_motion`, `talk_motion_clip`, `talk_clip`, `idle_motion_clip`, `idle_clip`, `viseme_motion_clip`, and `viseme_clip` are also accepted; node `metadata` may use the same keys as defaults for the current speaker.

Dialogue text can also trigger the same exported Live2D frame selection mid-line with hidden event tags. `[live2d_pose tag="surprised"]`, `[live2d_pose hint="surprised"]`, or shorthand `[live2d_pose=surprised]` targets the current speaker unless `id`, `speaker`, `target`, or `character_id` is supplied. `[live2d_motion clip="idle_loop" progress=0.4]` or shorthand `[live2d_motion=idle_loop]` selects an exported frame from a clip; add `loop=true`, `live2d_motion_autoplay=true`, or `live2d_loop=true` to keep cycling that clip afterward. Supported attributes mirror stage cast keys: `tag`, `tags`, `pose_tag`, `hint`, `clip`, `time`, `progress`, `loop`, `dialogue_motion`, `live2d_auto_dialogue_motion`, `idle_clip`, `talk_clip`, `viseme_clip`, `speed`, `blend`, `live2d_motion_blend_duration`, `portrait`, and `animation_speed`.

Web rig hit areas exported to `metadata.live2d_web_model.hit_areas[]` are normalized into the active portrait state as `live2d_hit_areas`. When the source part image is available during export, each area also includes canvas-space `bounds` / `points` and normalized `normalized_bounds` / `normalized_points` derived from the part's world transform and mesh vertices. Motion-frame swaps preserve this field, so later click/touch handlers can use stable area ids such as `head`, `face`, or `body` without re-reading the character file. If a recovered or older character has a `live2d_model` rig path but no mirrored metadata summary, the runtime can read the `.live2d-web.json` rig directly and recover enabled `imageParts[].hitArea` entries.

Runtime hit testing is exposed through `WebRigRuntime.hit_test_state_hit_areas(state, stage_position, display_rect)` and `story_dialogue_screen.gd` wrapper methods `get_live2d_hit_area_at_position(stage_position, speaker_id)` / `get_live2d_hit_areas_for_speaker(speaker_id)`. These use normalized polygon data when available, then fall back to normalized bounds. The current exported expression source can be read with `WebRigRuntime.get_expression_preset_from_state(state)` or `get_live2d_expression_preset_for_speaker(speaker_id)`.

Parameter influence summaries exported to `metadata.live2d_web_model.parameter_bindings[]` are normalized into the active portrait state as `live2d_parameter_bindings`. Each binding records the driven parameter, affected channels such as `x`, `rotation`, `visibility`, `draw_order`, `deformer_group`, `warp`, `mesh`, `physics`, or `motion`, affected image-part ids, and source counts for direct bindings, visibility gates, transform keys, draw order keys, deformer groups, warp deformers, mesh keys, physics rules, and motion keys. This gives runtime systems a compact way to understand what the generated rig can react to. When the summary is absent but the active frame has a readable `.live2d-web.json`, the runtime rebuilds the same lightweight binding summary from image parts, visibility gates, transform/draw-order/mesh keys, deformer groups, warp deformers, physics rules, and motion clips.

The character editor groups exported `live2d_motion_frame` portraits into a `Live2D motion set` summary and also reads `metadata.live2d_web_model.motion_frame_sets[]` when present, then plays the selected clip's exported frames inline so you can confirm which clips are available after exporting from `tools/live2d-editor`. It checks the saved character JSON for changed Live2D exports when the character opens; use `Sync Live2D exports` after saving from the web rig editor to merge changed generated Live2D portraits and `metadata.live2d_web_model` source information from disk back into the current character draft without replacing ordinary hand-authored portraits or unrelated metadata. If a repaired, imported, or not-yet-saved character draft has Live2D generated states only under `metadata.live2d_web_model.portraits` or `motion_frame_sets[].states`, the same sync action rebuilds top-level portrait records directly from local metadata so they appear in the portrait list. The dialogue editor exposes the same controls on each stage cast row when that character has exported frames or motion frame sets: `Live2D auto pose`, pose hint preset/custom input, motion loop, dialogue motion, motion clip, idle/talk/viseme clips, motion time, motion progress, motion speed, motion blend, and an auto-cycle reset. Use `Dialogue defaults` to automatically enable adaptive pose; when `dialogue_motion_set.ready` is true, it also enables dialogue motion and fills detected `idle_loop` / `talk_loop` / `viseme_set` clips. When a character summary includes `metadata.live2d_web_model.dialogue_motion_set`, adding that character to `stage_cast` or rebuilding the speaker's cast entry also seeds `adaptive_live2d_pose`; `live2d_dialogue_motion` and idle/talk/viseme clip defaults are seeded only after the dialogue motion export is complete. The stage preview resolves the same automatic frame when the cast entry leaves `portrait` empty, and dialogue motion previews prefer the idle clip.

`tools/live2d-editor` can generate an `adaptive_pose` still-frame set with `Export Adaptive Poses`, baseline `idle_loop` / `talk_loop` clips with `Auto Idle`, `Auto Talk`, or the one-pass `Export Dialogue Set` action, and expression preset states with `Export Presets`. Those exported frames are ordinary portrait states with `live2d_motion_frame` metadata, so no additional runtime format is required.

When a motion loop starts, the runtime preloads that loop's exported frame textures into the portrait cache before cycling them. This keeps the first visible loop from blocking on individual frame loads during dialogue.
...
[live2d_pose hint="surprised"]
[live2d_motion clip="idle_loop" progress=0.4 loop=true]
```
...
- `live2d` / `live2d_pose` / `live2d_motion`: changes an exported web-rig portrait frame when the typewriter reaches the tag. Supported target attributes are `id`, `speaker`, `target`, or `character_id`; if omitted, the current speaker is used. Pose attributes include `tag`, `pose_tag`, `hint`, `pose`, `emotion`, and `tags`; motion attributes include `clip`, `time`, `progress`, `loop`, `live2d_motion_autoplay`, `live2d_loop`, `dialogue_motion`, `live2d_auto_dialogue_motion`, `idle_clip`, `talk_clip`, `speed`, and `blend`.
