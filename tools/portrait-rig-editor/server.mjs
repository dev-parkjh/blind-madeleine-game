...
const maxBodyBytes = Math.max(40, Number(process.env.PORTRAIT_RIG_MAX_BODY_MB || 160)) * 1024 * 1024;
...
const godotImportAuto = readBooleanEnv(process.env.PORTRAIT_RIG_GODOT_IMPORT_AUTO, true);
...
function summarizeCharacter(id, data) {
  ...
  const metadata = data?.metadata && typeof data.metadata === "object" && !Array.isArray(data.metadata)
    ? data.metadata
    : {};
  const portraitRigMetadata = portraitRigMetadataFromMetadata(metadata);
  return {
    id,
    title: titleFromCharacter(data, id),
    nameColor: String(data?.name_color || "").trim() || "#ffffff",
    portraitKeys: Object.keys(portraits),
    hasWebRig: Boolean(portraitRigMetadata)
  };
}

function portraitRigMetadataFromMetadata(metadata) {
...
function summarizeMotionFrameSetsForMetadata(portraitRigPortraits) {
  const source = portraitRigPortraits && typeof portraitRigPortraits === "object" && !Array.isArray(portraitRigPortraits)
    ? portraitRigPortraits
    : {};
  const groups = new Map();
  for (const [state, entry] of Object.entries(source)) {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) continue;
    const frame = normalizeMotionFrameMetadata(entry.motion_frame || entry.motionFrame || entry.portrait_rig_motion_frame || entry.portraitRigMotionFrame);
    if (!frame) continue;
    ...
    const expressionPreset = normalizeExpressionPresetMetadata(entry.expression_preset || entry.expressionPreset || entry.portrait_rig_expression_preset || entry.portraitRigExpressionPreset);
    existing.states.push({
      ...
      model_path: String(entry.model_path || entry.modelPath || entry.portrait_rig_model || entry.portraitRigModel || ""),
...
function mergeMotionFrameSetStatesIntoPortraitsForSave(portraitRigPortraits, portraitRigMetadata) {
  const next = portraitRigPortraits && typeof portraitRigPortraits === "object" && !Array.isArray(portraitRigPortraits)
    ? { ...portraitRigPortraits }
    : {};
  const frameSets = [
  ...
  for (const rawFrameSet of frameSets) {
    ...
    for (const rawState of states) {
      ...
      const modelPath = String(sourceState.model_path || sourceState.modelPath || sourceState.portrait_rig_model || sourceState.portraitRigModel || "").trim();
      if (!String(current.image_path || current.imagePath || current.path || "").trim() && imagePath) {
        current.image_path = imagePath;
      }
      if (!String(current.model_path || current.modelPath || current.portrait_rig_model || current.portraitRigModel || "").trim() && modelPath) {
      ...
      const currentFrame = normalizeMotionFrameMetadata(current.motion_frame || current.motionFrame || current.portrait_rig_motion_frame || current.portraitRigMotionFrame);
      if (!currentFrame) {
        const fallbackFrame = normalizeMotionFrameMetadata(sourceState.motion_frame || sourceState.motionFrame || sourceState.portrait_rig_motion_frame || sourceState.portraitRigMotionFrame || {
          clip_id: clipId,
      ...
      const currentExpression = normalizeExpressionPresetMetadata(current.expression_preset || current.expressionPreset || current.portrait_rig_expression_preset || current.portraitRigExpressionPreset);
      if (!currentExpression) {
        const expressionPreset = normalizeExpressionPresetMetadata(sourceState.expression_preset || sourceState.expressionPreset || sourceState.portrait_rig_expression_preset || sourceState.portraitRigExpressionPreset);
...
function normalizePortraitRigMetadataPortraitsForSave(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return Object.fromEntries(Object.entries(source).flatMap(([state, rawEntry]) => {
    ...
    const modelPath = String(entry.model_path || entry.modelPath || entry.portrait_rig_model || entry.portraitRigModel || "").trim();
    if (imagePath) entry.image_path = imagePath;
    if (modelPath) entry.model_path = modelPath;
    const motionFrame = normalizeMotionFrameMetadata(entry.motion_frame || entry.motionFrame || entry.portrait_rig_motion_frame || entry.portraitRigMotionFrame);
...
async function preparePortraitRigPortraitSave(id, character, body, savedAt = new Date().toISOString()) {
  ...
  const modelRelativePath = path.posix.join(relativeDir, `${state}.portrait-rig.json`);
  ...
  portraits[state] = {
    ...
    center,
    profile,
    portrait_rig_model: `res://${modelRelativePath}`,
    generated_by: "tools/portrait-rig-editor"
  };
  if (motionFrame) portraits[state].portrait_rig_motion_frame = motionFrame;
  ...
  if (expressionPreset) portraits[state].portrait_rig_expression_preset = expressionPreset;

  const metadata = character.metadata && typeof character.metadata === "object" && !Array.isArray(character.metadata)
    ? { ...character.metadata }
    : {};
  const sourcePortraitRigMetadata = portraitRigMetadataFromMetadata(metadata);
  const portraitRigMetadata = sourcePortraitRigMetadata ? { ...sourcePortraitRigMetadata } : {};
  const portraitRigPortraits = mergeMotionFrameSetStatesIntoPortraitsForSave(
    normalizePortraitRigMetadataPortraitsForSave(portraitRigMetadata.portraits),
    portraitRigMetadata
  );
  portraitRigPortraits[state] = {
  ...
  if (motionFrame) portraitRigPortraits[state].motion_frame = motionFrame;
  if (expressionPreset) portraitRigPortraits[state].expression_preset = expressionPreset;
  ...
  const motionFrameSets = summarizeMotionFrameSetsForMetadata(portraitRigPortraits);
  ...
  const runtimeReadiness = summarizeRuntimeReadinessForMetadata({
    ...
    sourceCounts: {
      ...sourceCounts,
      portrait_count: Object.keys(portraitRigPortraits).length
    }
  });
  delete metadata.portraitRig;
  metadata.portrait_rig = {
    ...portraitRigMetadata,
    version: 1,
    ...
    portraits: portraitRigPortraits
  };
...
async function savePortraitRigPortrait(characterId, body) {
  const id = assertSafeId(characterId, "character id");
  const character = await loadCharacter(id);
  const prepared = await preparePortraitRigPortraitSave(id, character, body);
...
async function savePortraitRigPortraitBatch(characterId, body) {
  const id = assertSafeId(characterId, "character id");
  ...
  for (let index = 0; index < frames.length; index += 1) {
    ...
    const prepared = await preparePortraitRigPortraitSave(id, character, frameBody, savedAt);
...
async function savePortraitRigPart(characterId, body) {
  const id = assertSafeId(characterId, "character id");
  ...
  const relativePath = path.posix.join("assets", "characters", id, "portrait_rig_parts", `${partId}.${extension}`);
...
async function handleApi(request, response, url) {
  ...
  const batchSaveMatch = url.pathname.match(/^\/api\/characters\/([^/]+)\/portraitRig-portrait-batch$/);
  if (batchSaveMatch && request.method === "POST") {
    sendJson(response, 200, await savePortraitRigPortraitBatch(batchSaveMatch[1], await readJsonBody(request)));
    return true;
  }

  const saveMatch = url.pathname.match(/^\/api\/characters\/([^/]+)\/portraitRig-portrait$/);
  if (saveMatch && request.method === "POST") {
    sendJson(response, 200, await savePortraitRigPortrait(saveMatch[1], await readJsonBody(request)));
    return true;
  }

  const partMatch = url.pathname.match(/^\/api\/characters\/([^/]+)\/portraitRig-part$/);
  if (partMatch && request.method === "POST") {
    sendJson(response, 201, await savePortraitRigPart(partMatch[1], await readJsonBody(request)));
    return true;
  }
