# Blind Madeleine Portrait Rig Editor

Standalone web project for building layered, parameter-driven character portraits and saving them as realtime game data.

This project is a clean-room, game-specific rig editor. It does not use third-party proprietary rig sources, assets, file formats, or reverse-engineered internals.

## Run

When the local data editor runs from `tools/editor`, it starts this portrait rig editor automatically by default and exposes the URL through `/api/health`.

```bash
cd tools/portrait-rig-editor
npm run start
```
...
The check command validates the server and browser script syntax, then runs a headless smoke test against a temporary local server. The smoke test creates a throwaway character, saves a generated Portrait Rig portrait through `/api/characters/{id}/portraitRig-portrait`, verifies the character JSON, saved rig model files, motion metadata, editable parts, hit areas, and parameter bindings, then removes the temporary files.
...
The editor's primary runtime artifact is:

```text
assets/characters/{character_id}/{portrait_state}.portrait-rig.json
```
...
The selected portrait state is stored under `portraits.{state}` with face center, profile crop, and a `portrait_rig_model` path. A compatibility image path may still be present for older editor previews, but the Godot runtime prefers `portrait_rig_model` and renders the parts, parameters, physics, and motion data directly. Rig metadata is also mirrored under `metadata.portrait_rig.portraits`, with a lightweight source index containing the last `source_model_path`, generated portrait count, image part count, deformer group, auto deformer group, nested deformer, warp deformer, visibility gate, and locked part counts, hit area summaries, parameter count, parameter binding summaries, expression preset summaries, motion clip summaries, motion sample sets, dialogue motion defaults, and the preferred adaptive clip id.

Use `Rig Check` before saving or exporting to catch common JSON issues: missing image paths, broken parent or clip mask references, missing parameter bindings, malformed mesh vertex counts, empty motion clips, invalid physics targets, and duplicate preset ids. The panel also summarizes production readiness across parts, driven parameters, deformers, adaptive motion samples, and dialogue clips so the creator can see whether the rig is ready for game dialogue. Errors block `Save Portrait`, `Export Rig`, preset export, and motion sample export; warnings are shown but do not block export.

Imported image parts are stored as editable rig source assets:

```text
assets/characters/{character_id}/portrait_rig_parts/{part_id}.png
```

These parts are included in the saved `.portrait-rig.json` as `imageParts`. Each part can be placed in front of or behind the generated model, transformed independently, and bound to parameters for X/Y movement, rotation, scale, and opacity during motion preview and runtime rendering.

Use `Export Project` to save a portable `.portrait-rig-project.json` bundle containing the rig JSON plus embedded image-part data URLs. `Import Project` restores that bundle into the currently selected character by re-uploading each embedded image part through the editor server and rewriting `imageParts[].path` to the restored `res://assets/characters/{character_id}/portrait_rig_parts/...` files. This is the source-project backup path; `Export Rig` remains a lightweight JSON reference export and does not embed images.
...
Use `Import Layers` when separated artwork comes with a clean-room layer manifest. Select the image files together with `layers.json` or `manifest.json`; each manifest row can point at a file by `file`, `filename`, `image`, `imagePath`, or `path`, and can include `id`, `label`, `x`/`y` or `bounds`, `width`/`height`, `order`, `slot`, `parent`, `clipMask`, `opacity`, `blendMode`, bindings, visibility gates, hit areas, transform keys, draw-order keys, or mesh data. The importer uploads the images through the same `/portraitRig-part` asset path as normal image parts, applies name-based auto rigging, then reapplies manifest placement and rig data so external layer exports can become editable portrait rigs quickly without using proprietary PSD or Portrait Rig formats. Use `Export Layers` to write the current image-part stack back to `*.layers.json`; `Export Project` also embeds that same `layerManifest` beside the rig and image data URLs for source handoff.
...
`Rig Check` also reports runtime-readiness warnings for Portrait Rig dialogue export, including missing motion clips, missing `adaptive_pose`, disabled adaptive parameters, too few adaptive export frames, low pose tag diversity, and weak hint matches for common dialogue cues.
...
Use `Export JSON` to save the selected motion clip as a portable clean-room motion file, or `Export All JSON` to save every clip in one file. `Import JSON` accepts those files, a full `.portrait-rig.json` rig, or a raw `motionClips[]` / `clips[]` array, then appends imported clips without overwriting existing ids.
...
Enable `Onion skin` in the Motion panel to draw the previous and next sampled motion poses as translucent ghosts around the current time. `Ghost step` controls the time offset used for those samples and wraps at clip boundaries, making idle/talk loop seams easier to inspect. Onion skin is preview-only and is not saved into runtime rigs.
...
Use `Auto Viseme` to generate a `viseme_set` clip with closed, A, I/E, O, U, and closed mouth poses. Exported frames receive `pose_tags` / `pose_score` values such as `viseme_a`, `viseme_i`, `viseme_o`, `viseme_u`, `viseme_closed`, `viseme`, and `phoneme`, so dialogue data can target them through `portrait_rig_pose_hint` or by selecting `viseme_set` as a motion clip.

Use `Export Frames` to sample the selected motion clip into named portrait states for editor compatibility and pose metadata. A clip named `idle_loop` registers states such as `idle_loop_f01`, `idle_loop_f02`, and so on in `portraits.{state}` while keeping the shared `portrait_rig_model` path. `Clip export frames` stores the selected clip's preferred `motionClips[].exportFrames` count and is also used by batch export. If physics is enabled, each sample evaluates `physics.rules[]` at the frame time after applying the motion keyframes.
...
Use `Export Adaptive Poses` to generate an `adaptive_pose` clean-room pose set and save dialogue-ready pose samples. The generator starts from safe baseline head, eye, brow, mouth, breath, and hair-sway poses, then analyzes the current rig's direct part bindings, transform keys, mesh deformers, physics rules, and existing motion keys. Custom parameters that actually drive the rig receive deterministic pose variation automatically. When a dialogue cast has no explicit `portrait_rig_motion_clip`, the main editor preview and game runtime prefer `adaptive_pose` before falling back to idle or talk loops, so ordinary dialogue advance gets varied poses without hand-authoring a clip first.

Adaptive pose tuning is saved under `adaptivePose` and mirrored into character metadata as `metadata.portrait_rig.adaptive_pose_tuning`. `Pose energy` scales the generated pose deltas, while the adaptive rig map lists the driven parameters, semantic role, estimated amplitude, influence score, and source channels. Uncheck a parameter to exclude it from future `Build Adaptive` and `Export Adaptive Poses` generation without deleting its rig bindings.
...
Use `Export Dialogue Set` to generate `idle_loop`, `talk_loop`, `viseme_set`, and `adaptive_pose`, then save their recommended sample counts in one pass. The adaptive clip is generated after the baseline dialogue clips so it can see the rig's existing motion ranges, and export writes `metadata.portrait_rig.dialogue_motion_set` with the default adaptive, idle, talk, and viseme clip ids. The resulting rig data is immediately usable by the main editor's Portrait Rig stage-cast controls and by the game runtime's adaptive pose selection, dialogue idle/talk switching, and typewriter-driven viseme targeting.

Use `Export Current` to render only the current timeline time as one portrait state. The default state id is `{clip_id}_t{milliseconds}`, and the exported portrait keeps the same `portrait_rig_motion_frame` metadata so dialogue can target it by `portrait_rig_motion_time`.

Exported motion samples include `portrait_rig_motion_frame` portrait metadata with `clip_id`, `time`, `frame_index`, `frame_count`, `clip_duration`, `physics_sampled`, semantic `pose_tags`, `pose_score`, and sampled `parameter_values`. The character metadata also receives `metadata.portrait_rig.motion_frame_sets[]`, grouping those states by clip id with their state keys, frame times, model paths, crop data, and pose metadata, plus `dialogue_motion_set` so the editor and runtime can use the same adaptive/idle/talk/viseme defaults. The game runtime reads the rig model and metadata through `scripts/visual_novel/web_rig_runtime.gd`: when a dialogue line advances and the speaker has no explicit portrait override, it can match `portrait_rig_pose_hint` / inferred dialogue text cues to the best tagged sample, prefer a candidate whose `parameter_values` differ from the currently visible sample, penalize recently used equivalent samples to avoid repeated poses, then render the selected pose through the realtime portrait rig renderer. During dialogue typing, `viseme_set` samples tagged as `viseme_a`, `viseme_i`, `viseme_o`, `viseme_u`, or `viseme_closed` are selected from the current visible character before falling back to the talk loop. Runtime motion loops use `clip_duration` when available, then fall back to estimating the loop length from sample times for older exports.

Dialogue data can also target a clip time directly. Set `stage_cast.{character_id}.portrait_rig_motion_clip` plus `portrait_rig_motion_time` seconds or `portrait_rig_motion_progress` from `0` to `1`; the runtime selects the nearest motion sample for that clip. If time/progress is omitted, use `portrait_rig_pose_hint` values such as `happy`, `sad`, `angry`, `surprised`, `curious`, `talk`, or `blink` to select by pose tags. The same keys may be placed in node `metadata` as defaults for the current speaker.
...
If the Godot preview bridge is available at `GODOT_PREVIEW_ENDPOINT` or `http://127.0.0.1:51234`, the server can also request a Godot asset import for refreshed preview assets. Disable that with:

```bash
PORTRAIT_RIG_GODOT_IMPORT_AUTO=0 npm run start
```

## Editor Handoff

Saved rig portraits are regular character portraits. In `tools/editor`, open the character portrait list, refresh/load the character, and the portrait appears with crop controls and its `portrait_rig_model` source path. The `초상 리그 편집` button opens this standalone editor for the same character and portrait state:

```text
http://127.0.0.1:5187/?character={character_id}&portrait={portrait_state}
```

Inside the standalone editor, use `Existing portrait` to pick one of the character's saved portrait states. If that state has a saved portrait rig, the editor reopens the matching `.portrait-rig.json`; otherwise it only switches the `Portrait state` field. When opening a character or a new state, the editor can also fall back to `metadata.portrait_rig.source_model_path`, so newly generated states start from the latest saved rig source. `Load State Rig` still reloads the exact state manually using the `portrait_rig_model` path stored on `portraits.{state}`, the mirrored `metadata.portrait_rig.portraits.{state}.model_path`, or a matching `metadata.portrait_rig.motion_frame_sets[].states[].model_path` entry. This keeps repaired/imported character files usable even when the top-level `portraits` map has not been rebuilt yet.
