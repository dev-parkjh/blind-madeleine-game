...
import { portraitRigStageCastDefaultsForCharacterId } from "../src/features/dialogues/stageCastPortraitRigDefaults.ts";
...
const characters = [{
  ...
  validation: {
    portraitRigMotionClipIds: ["adaptive_pose", "idle_loop", "talk_loop", "viseme_set"],
    portraitRigPoseTags: ["neutral", "talk", "surprised", "viseme_a"],
...
const camelCharacters = [{
  ...
  subtitle: "Portrait Rig",
  validation: {
    portraitRigMotionClipIds: ["camel_adaptive", "camel_idle", "camel_talk", "camel_viseme"],
    portraitRigPoseTags: ["neutral", "curious"],
    portraitRigDialogueMotion: {
      ready: true,
      adaptiveClipId: "camel_adaptive",
      idleClipId: "camel_idle",
...
const portraitRigDefaults = portraitRigStageCastDefaultsForCharacterId(characterId, characters);
assert.deepEqual(portraitRigDefaults, {
...
assert.deepEqual(portraitRigStageCastDefaultsForCharacterId("incomplete_madeleine", incompleteCharacters), {
  adaptive_portrait_rig_pose: true
});
...
assert.equal(readySpeakerNode.stage_cast[characterId].adaptive_portrait_rig_pose, true);
assert.equal(readySpeakerNode.stage_cast[characterId].portrait_rig_dialogue_motion, true);
assert.equal(readySpeakerNode.stage_cast[characterId].portrait_rig_idle_motion_clip, "idle_loop");
...
assert.equal(incompleteSpeakerNode.stage_cast.incomplete_madeleine.adaptive_portrait_rig_pose, true);
assert.equal(incompleteSpeakerNode.stage_cast.incomplete_madeleine.portrait_rig_dialogue_motion, undefined);
assert.equal(incompleteSpeakerNode.stage_cast.incomplete_madeleine.portrait_rig_idle_motion_clip, undefined);
...
assert.equal(incompleteCleanup.node.stage_cast.incomplete_madeleine.adaptive_portrait_rig_pose, true);
assert.equal(incompleteCleanup.node.stage_cast.incomplete_madeleine.portrait_rig_dialogue_motion, undefined);
...
assert.equal(incompleteEntry.portraitRigDialogueMotionReady, false);

const metadataHintEntry = buildEntry(
  { [characterId]: {} },
  { speaker: characterId, metadata: { portrait_rig_pose_hint: "surprised" }, text: "" }
);
assert.equal(metadataHintEntry.portraitRigPoseHint, "surprised");
...
assert.equal(inferredHintEntry.portraitRigPoseHint.includes("surprised"), true);
assert.equal(inferredHintEntry.portraitRigPoseHint.includes("talk"), true);
assert.equal(inferredHintEntry.portraitRigPoseHint.includes("viseme_u"), true);
assert.equal(inferredHintEntry.portrait?.key, "surprised");

const motionLoopEntry = buildEntry(
  { [characterId]: { portrait_rig_motion_loop: true } },
  { speaker: characterId, text: "" }
);
assert.deepEqual(motionLoopEntry.portraitRigMotionPreviewFrames.map((frame) => frame.key), ["neutral", "surprised", "talk"]);
assert.equal(motionLoopEntry.portraitRigMotionPreviewDuration, 1.2);

const dialogueMotionEntry = buildEntry(
  { [characterId]: { portrait_rig_dialogue_motion: true } },
  { speaker: characterId, text: "" }
);
assert.deepEqual(dialogueMotionEntry.portraitRigMotionPreviewFrames.map((frame) => frame.key), ["idle_01", "idle_02"]);
assert.equal(dialogueMotionEntry.portraitRigMotionPreviewDuration, 1);
assert.equal(dialogueMotionEntry.portraitRigDialogueMotionReady, true);
assert.equal(dialogueMotionEntry.portraitRigSuggestedIdleMotionClip, "idle_loop");
assert.equal(dialogueMotionEntry.portraitRigSuggestedTalkMotionClip, "talk_loop");
assert.equal(dialogueMotionEntry.portraitRigSuggestedVisemeMotionClip, "viseme_set");
assert.equal(dialogueMotionEntry.portraitRigMotionClips.some((clip) => clip.value === "talk_loop"), true);
assert.equal(dialogueMotionEntry.portraitRigMotionClips.some((clip) => clip.value === "viseme_set"), true);
assert.equal(dialogueMotionEntry.portraitRigPoseTags.includes("worried"), true);
assert.equal(dialogueMotionEntry.portraitRigPoseTags.includes("anxious"), true);

const camelDialogueMotionEntry = buildEntryFor(
  camelCharacterId,
  camelCharacter,
  camelCharacters,
  { [camelCharacterId]: { portrait_rig_dialogue_motion: true } },
  { speaker: camelCharacterId, text: "" }
);
assert.equal(camelDialogueMotionEntry.portraitRigSuggestedIdleMotionClip, "camel_idle");
assert.deepEqual(camelDialogueMotionEntry.portraitRigMotionPreviewFrames.map((frame) => frame.key), ["camel_idle_01", "camel_idle_02"]);
assert.equal(camelDialogueMotionEntry.portraitRigPoseTags.includes("curious"), true);
...
assert.equal(camelStageCastAliasEntry.portraitRigDialogueMotion, true);
assert.equal(camelStageCastAliasEntry.portraitRigIdleMotionClip, "camel_idle");
assert.equal(camelStageCastAliasEntry.portraitRigPoseHint, "curious");
assert.equal(camelStageCastAliasEntry.portraitRigMotionProgress, 0.5);
assert.equal(camelStageCastAliasEntry.portraitRigMotionSpeed, 1.7);
assert.equal(camelStageCastAliasEntry.portraitRigMotionBlendDuration, 0.22);
assert.equal(camelStageCastAliasEntry.portrait?.key, "camel_idle_02");
assert.deepEqual(camelStageCastAliasEntry.portraitRigMotionPreviewFrames.map((frame) => frame.key), ["camel_idle_01", "camel_idle_02"]);

const camelMotionLoopAliasEntry = buildEntryFor(
  camelCharacterId,
  camelCharacter,
  camelCharacters,
  { [camelCharacterId]: { motionLoop: true, portraitRigMotionClip: "camel_idle", motionTime: 0.5 } },
  { speaker: camelCharacterId, text: "" }
);
assert.equal(camelMotionLoopAliasEntry.portraitRigMotionLoop, true);
assert.equal(camelMotionLoopAliasEntry.portraitRigMotionClip, "camel_idle");
assert.equal(camelMotionLoopAliasEntry.portraitRigMotionTime, 0.5);
assert.equal(camelMotionLoopAliasEntry.portrait?.key, "camel_idle_02");
assert.deepEqual(camelMotionLoopAliasEntry.portraitRigMotionPreviewFrames.map((frame) => frame.key), ["camel_idle_01", "camel_idle_02"]);
...
assert.equal(camelMetadataAliasEntry.portraitRigDialogueMotion, true);
assert.equal(camelMetadataAliasEntry.portraitRigIdleMotionClip, "camel_idle");
assert.equal(camelMetadataAliasEntry.portraitRigPoseHint, "curious");
assert.equal(camelMetadataAliasEntry.portrait?.key, "camel_idle_02");
assert.deepEqual(camelMetadataAliasEntry.portraitRigMotionPreviewFrames.map((frame) => frame.key), ["camel_idle_01", "camel_idle_02"]);

const dialogueVisemeMotionEntry = buildEntry(
  { [characterId]: { portrait_rig_dialogue_motion: true } },
  { speaker: characterId, text: "오오!" }
);
assert.deepEqual(dialogueVisemeMotionEntry.portraitRigMotionPreviewFrames.map((frame) => frame.key), [
  "viseme_closed",
  "viseme_a",
  "viseme_o",
  "viseme_u"
]);
assert.equal(dialogueVisemeMotionEntry.portraitRigMotionPreviewFrames.every((frame) => frame.clipId === "viseme_set"), true);
assert.equal(dialogueVisemeMotionEntry.portraitRigMotionPreviewDuration, 1.2);

console.log("Stage cast Portrait Rig preview smoke test passed.");
...
function frameState(state, clipId, frameIndex, time, poseTags, poseScore, parameterValues) {
  return {
    ...
    model_path: `res://assets/characters/${characterId}/${state}.portrait-rig.json`,
    center: [0.5, 0.22],
...
function camelFrameState(state, clipId, frameIndex, time, poseTags, poseScore, parameterValues) {
  return {
    ...
    modelPath: `res://assets/characters/${camelCharacterId}/${state}.portrait-rig.json`,
    center: [0.5, 0.22],
    motionFrame: {
      clipId,
      clipLabel: clipId,
