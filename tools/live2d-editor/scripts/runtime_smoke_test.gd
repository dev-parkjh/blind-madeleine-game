...
func _init() -> void:
	...
	_expect(String(surprised.get("live2d_model", "")) == "res://assets/characters/madeleine/surprised.live2d-web.json", "metadata-only rig path missing")
	_expect(Vector2(surprised.get("center", Vector2.ZERO)).distance_to(Vector2(0.52, 0.2)) < 0.001, "metadata-only center missing")
	_expect((surprised.get("live2d_hit_areas", []) as Array).size() == 1, "hit areas were not attached to frame state")
	var surprised_bindings: Array[Dictionary] = []
	for raw_binding in surprised.get("live2d_parameter_bindings", []) as Array:
	...
	var surprised_preset: Dictionary = surprised.get("live2d_expression_preset", {})
	...
	var state_expression_live2d: Dictionary = state_expression_profile["metadata"]["live2d_web_model"]
	var state_expression_sets: Array = state_expression_live2d["motion_frame_sets"]
	...
	_expect(String((state_expression_talk.get("live2d_expression_preset", {}) as Dictionary).get("id", "")) == "talk_state_expr", "motion frame state expression preset was not attached")
	var global_expression_profile := _build_profile()
	var global_expression_live2d: Dictionary = global_expression_profile["metadata"]["live2d_web_model"]
	global_expression_live2d["expression_presets"] = [
		{
			"id": "talk_global_expr",
	...
	_expect(String((global_expression_talk.get("live2d_expression_preset", {}) as Dictionary).get("id", "")) == "talk_global_expr", "global expression preset fallback did not match motion frame pose tags")
	...
	var source_model_path := "user://live2d_runtime_source_rig.json"
	...
	if source_file == null:
		...
		_expect(String(source_model_frame.get("live2d_model", "")) == source_model_path, "source_model_path was not used as a frame rig fallback")
		_expect((source_model_frame.get("live2d_hit_areas", []) as Array).size() == 1, "source_model_path rig did not backfill hit areas")
		var source_model_bindings: Array[Dictionary] = []
		for raw_binding in source_model_frame.get("live2d_parameter_bindings", []) as Array:
			if typeof(raw_binding) == TYPE_DICTIONARY:
	...
	_expect(neutral_to_surprised_distance > 0.25 and neutral_to_surprised_distance <= 1.0, "parameter pose distance did not detect normalized Live2D parameter change")
	...
	var mouth_only_distance := WebRigRuntime.get_motion_frame_parameter_distance({
		...
		"live2d_parameter_bindings": role_weight_state["live2d_parameter_bindings"],
	}, role_weight_state)
	...
	var role_only_live2d: Dictionary = role_only_profile["metadata"]["live2d_web_model"]
	role_only_live2d.erase("parameter_bindings")
	role_only_live2d["parameter_roles"] = [
		{"parameter": "mouthOpen", "label": "Mouth Open", "role": "mouth_open"},
		{"parameter": "angleX", "label": "Angle X", "role": "gaze_x"},
	]
	...
	for raw_binding in role_only_surprised.get("live2d_parameter_bindings", []) as Array:
		if typeof(raw_binding) == TYPE_DICTIONARY:
	...
	var expression_only_distance := WebRigRuntime.get_motion_frame_parameter_distance(
		{"live2d_expression_preset": {"id": "happy", "label": "Happy"}},
		{"live2d_expression_preset": {"id": "neutral", "label": "Neutral"}}
	)
	...
	_expect(String(portrait_entry.get("live2d_model", "")) == "res://assets/characters/madeleine/surprised.live2d-web.json", "PortraitLayout did not resolve metadata-only rig")
	var portrait_frame: Dictionary = portrait_entry.get("live2d_motion_frame", {})
	_expect(String(portrait_frame.get("clip_id", "")) == "adaptive_pose", "PortraitLayout did not resolve metadata-only motion frame")
	var portrait_expression_preset: Dictionary = portrait_entry.get("live2d_expression_preset", {})
	...
	_expect(String(camel_pose.get("live2d_model", "")) == "res://assets/characters/madeleine/camel_pose.live2d-web.json", "camelCase frame set modelPath was not read")
	...
	_expect((camel_pose.get("live2d_hit_areas", []) as Array).size() == 1, "camelCase hitAreas were not attached to frame state")
	_expect((camel_pose.get("live2d_parameter_bindings", []) as Array).size() == 1, "camelCase parameterBindings were not attached to frame state")
	_expect(String((camel_pose.get("live2d_expression_preset", {}) as Dictionary).get("id", "")) == "camel_curious", "camelCase expressionPresets did not match frame pose tags")
	...
	_expect(String(camel_metadata_entry.get("live2d_model", "")) == "res://assets/characters/madeleine/camel_metadata.live2d-web.json", "PortraitLayout did not resolve camelCase metadata modelPath")
	var camel_metadata_frame: Dictionary = camel_metadata_entry.get("live2d_motion_frame", {})
	...
	var camel_hint_key := WebRigRuntime.select_next_motion_frame_portrait_key(
		camel_alias_profile,
		"madeleine",
		{"live2dMotionClip": "camel_adaptive", "poseHint": "curious"},
		{},
		"camel_dialogue",
		"camel_node",
		{},
	...
	_expect(String(camel_top_level_motion_set.get("adaptive_clip_id", "")) == "camel_adaptive", "top-level live2dWebModel dialogueMotionSet was not read")
	_expect(WebRigRuntime.get_hit_areas_from_profile(camel_top_level_profile).size() == 1, "top-level live2dWebModel hitAreas were not read")
	_expect(WebRigRuntime.get_parameter_bindings_from_profile(camel_top_level_profile).size() == 1, "top-level live2dWebModel parameterBindings were not read")
	_expect(WebRigRuntime.get_expression_presets_from_profile(camel_top_level_profile).size() == 1, "top-level live2dWebModel expressionPresets were not read")
	var camel_top_level_frames := WebRigRuntime.get_motion_frame_portraits(camel_top_level_profile)
	_expect(camel_top_level_frames.size() == 2, "top-level live2dWebModel motion frames were not collected")
	var camel_top_level_portrait := PortraitLayout.resolve_portrait_entry(camel_top_level_profile, "camel_metadata")
	_expect(String(camel_top_level_portrait.get("path", "")) == "res://assets/characters/madeleine/camel_metadata.png", "PortraitLayout did not resolve top-level live2dWebModel metadata portrait")
	...
	var ready_talk_key := WebRigRuntime.select_next_motion_frame_portrait_key(
		...
		"madeleine",
		{"live2d_motion_clip": "talk_loop", "live2d_pose_hint": "open_mouth"},
		neutral_state,
	...
	if _failed:
		quit(1)
		return
	print("Live2D Godot runtime smoke test passed.")
	quit(0)
				...
				"motion_frame_sets": [
					{
						...
						"states": [
							...
							},
							{
								...
								"model_path": "res://assets/characters/madeleine/surprised.live2d-web.json",
								"center": [0.52, 0.2],
							...
							},
							{
								...
								"model_path": "res://assets/characters/madeleine/talk.live2d-web.json",
								"center": [0.49, 0.21],
								"pose_tags": ["talk", "open_mouth"],
								"pose_score": {
									"talk": 0.96,
			...
			"live2d_web_model": {
						...
						"states": [
							{
								...
								"modelPath": "res://assets/characters/madeleine/camel_pose.live2d-web.json",
								"center": [0.48, 0.2],
								"poseTags": ["curious"],
								"poseScore": {
									"curious": 0.9,
				...
				"portraits": {
					"camel_metadata": {
						...
						"modelPath": "res://assets/characters/madeleine/camel_metadata.live2d-web.json",
						"center": [0.5, 0.21],
						"motionFrame": {
							"clipId": "camel_adaptive",
							"clipLabel": "Camel Adaptive",
...
func _build_camel_top_level_alias_profile() -> Dictionary:
	...
	metadata["live2dWebModel"] = metadata["live2d_web_model"]
	metadata.erase("live2d_web_model")
	return profile
...
func _build_profile_with_idle_loop() -> Dictionary:
	var profile := _build_profile()
	var live2d: Dictionary = profile["metadata"]["live2d_web_model"]
	var frame_sets: Array = live2d["motion_frame_sets"]
	frame_sets.append({
		"clip_id": "idle_loop",
		...
		"states": [
			{
				...
				"model_path": "res://assets/characters/madeleine/idle_01.live2d-web.json",
				"center": [0.5, 0.22],
				"pose_tags": ["neutral", "closed_mouth"],
				"pose_score": {
					"neutral": 1.0,
			...
			},
			{
				...
				"model_path": "res://assets/characters/madeleine/idle_02.live2d-web.json",
				"center": [0.5, 0.22],
				"pose_tags": ["neutral", "closed_mouth"],
				"pose_score": {
					"neutral": 0.95,
...
func _build_dialogue_ready_profile() -> Dictionary:
	...
	var live2d: Dictionary = profile["metadata"]["live2d_web_model"]
	var dialogue_motion_set: Dictionary = live2d["dialogue_motion_set"]
...
func _portrait_motion_entry(
	...
	return {
		...
		"live2d_model": "res://assets/characters/madeleine/%s.live2d-web.json" % state,
		"center": [0.5, 0.22],
		"live2d_motion_frame": {
			"clip_id": clip_id,
			"clip_label": clip_id,
...
func _talk_state(state: String, frame_index: int, time: float, pose_tags: Array, pose_score: Dictionary, mouth_open: float) -> Dictionary:
	return {
		...
		"model_path": "res://assets/characters/madeleine/%s.live2d-web.json" % state,
		"center": [0.5, 0.22],
		"pose_tags": pose_tags,
		"pose_score": pose_score,
		"parameter_values": {
...
func _build_profile_with_repeated_surprised_tags() -> Dictionary:
	...
	var repeated_frame := {
		...
		"model_path": "res://assets/characters/madeleine/surprised_alt.live2d-web.json",
		"center": [0.51, 0.2],
		"pose_tags": ["surprised", "open_mouth"],
		"pose_score": {
			"surprised": 0.98,
	...
	(live2d["portraits"] as Dictionary)["surprised_alt"] = {
		"image_path": "res://assets/characters/madeleine/surprised_alt.png",
		"model_path": "res://assets/characters/madeleine/surprised_alt.live2d-web.json",
		"center": [0.51, 0.2],
		"motion_frame": {
...
func _build_profile_with_duplicate_surprised_signature() -> Dictionary:
	...
	var duplicate_frame := {
		...
		"model_path": "res://assets/characters/madeleine/surprised_clone.live2d-web.json",
		"center": [0.52, 0.2],
		"pose_tags": ["surprised", "open_mouth"],
		"pose_score": {
			"surprised": 0.98,
	...
	(live2d["portraits"] as Dictionary)["surprised_clone"] = {
		"image_path": "res://assets/characters/madeleine/surprised_clone.png",
		"model_path": "res://assets/characters/madeleine/surprised_clone.live2d-web.json",
		"center": [0.52, 0.2],
		"motion_frame": {
...
func _build_profile_with_emotion_tags() -> Dictionary:
	...
	states.append({
		...
		"model_path": "res://assets/characters/madeleine/angry.live2d-web.json",
		"center": [0.5, 0.22],
		"pose_tags": ["angry"],
		"pose_score": {
			"angry": 1.0,
	...
	states.append({
		...
		"model_path": "res://assets/characters/madeleine/curious.live2d-web.json",
		"center": [0.5, 0.22],
		"pose_tags": ["curious"],
		"pose_score": {
			"curious": 1.0,
