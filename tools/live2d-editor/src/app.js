...
const draftStorageKey = "blind-madeleine-live2d-editor-draft";
...
function findSavedRigModelPath(character, preferredState = "", allowFallback = true) {
  ...
  const live2dMetadata = live2dMetadataFromCharacter(character);
  const portraits = live2dMetadata.portraits;
  if (portraits && typeof portraits === "object" && !Array.isArray(portraits)) {
    const preferred = modelPathFromLive2dPortrait(portraits[state]);
    if (preferred) return preferred;
    if (allowFallback) {
      const sourceModelPath = String(live2dMetadata.source_model_path || live2dMetadata.sourceModelPath || "").trim();
      if (sourceModelPath) return sourceModelPath;
      const defaultPath = modelPathFromLive2dPortrait(portraits.default);
      if (defaultPath) return defaultPath;
      for (const entry of Object.values(portraits)) {
        const modelPath = modelPathFromLive2dPortrait(entry);
        if (modelPath) return modelPath;
      }
    }
  }
  const frameSetStateModelPath = modelPathFromLive2dMotionFrameSets(live2dMetadata, state, false);
  if (frameSetStateModelPath) return frameSetStateModelPath;
  if (allowFallback) {
    const sourceModelPath = String(live2dMetadata.source_model_path || live2dMetadata.sourceModelPath || "").trim();
    if (sourceModelPath) return sourceModelPath;
    const fallbackFrameSetModelPath = modelPathFromLive2dMotionFrameSets(live2dMetadata, state, true);
  ...
  if (characterPortraits && typeof characterPortraits === "object" && !Array.isArray(characterPortraits)) {
    ...
    const preferredModelPath = modelPathFromLive2dPortrait(preferredEntry);
...
function live2dMetadataFromCharacter(character) {
...
function modelPathFromLive2dPortrait(entry) {
  if (!entry || typeof entry !== "object" || Array.isArray(entry)) return "";
  return String(entry.model_path || entry.modelPath || entry.live2d_model || entry.live2dModel || "").trim();
}

function modelPathFromLive2dMotionFrameSets(live2dMetadata, preferredState = "", allowFallback = false) {
  const states = live2dMotionFrameSetStates(live2dMetadata);
...
function live2dMotionFrameSetStates(live2dMetadata) {
  const frameSets = [
  ...
  for (const frameSet of frameSets) {
    ...
    for (const entry of sourceStates) {
      ...
      states.push({
        state,
        modelPath: String(entry.model_path || entry.modelPath || entry.live2d_model || entry.live2dModel || "").trim()
      });
    }
  }
  return states;
...
function portraitStateKeys(character = currentCharacter) {
  ...
  const live2dMetadata = live2dMetadataFromCharacter(character);
  const rigPortraits = live2dMetadata.portraits;
  ...
  for (const entry of live2dMotionFrameSetStates(live2dMetadata)) {
...
async function exportExpressionPresetsAsPortraits() {
  ...
  try {
    ...
    for (const [index, preset] of presets.entries()) {
      ...
      const result = await requestJson(`/api/characters/${encodeURIComponent(currentCharacter.id)}/live2d-portrait`, {
        method: "POST",
        body: JSON.stringify({
...
async function saveLive2dPortraitFrameModels(models) {
  ...
  return requestJson(`/api/characters/${encodeURIComponent(currentCharacter.id)}/live2d-portrait-batch`, {
    method: "POST",
...
async function exportMotionFramesAsPortraits() {
  ...
  elements.playMotion.disabled = true;
  try {
    const result = await saveLive2dPortraitFrameModels(frameTimes.map((time, index) => modelForMotionFrame(clip, time, index, "", frameCount)));
    currentCharacter = result.data;
    await loadCharacters(currentCharacter.id);
...
async function exportAllMotionFramesAsPortraits() {
  ...
  try {
    ...
    const result = await saveLive2dPortraitFrameModels(models);
    currentCharacter = result.data;
    await loadCharacters(currentCharacter.id);
...
async function exportRig() {
  ...
  annotateHitAreaBounds(model);
  downloadJson(
    `${safeSegment(rig.character.displayName, "character")}.${safeSegment(rig.portrait.state, "default")}.live2d-web.json`,
    model
  );
}
...
async function exportProjectBundle() {
  ...
  downloadJson(
    `${safeSegment(rig.character.displayName, "character")}.${safeSegment(rig.portrait.state, "default")}.live2d-web-project.json`,
    {
      app: "tools/live2d-editor",
      kind: "live2d_web_project",
      version: 1,
      exportedAt: new Date().toISOString(),
      character: {
...
function imagePartLayerManifest(model) {
  ...
  return {
    app: "tools/live2d-editor",
    kind: "live2d_web_layer_manifest",
    version: 1,
    exportedAt: new Date().toISOString(),
    canvas: {
...
function projectBundleRig(bundle) {
  ...
  if (bundle.kind === "live2d_web_project" && bundle.rig && typeof bundle.rig === "object" && !Array.isArray(bundle.rig)) {
...
async function restoreProjectBundleAssets(bundle, model) {
  ...
  for (const asset of assets) {
    ...
    const result = await requestJson(`/api/characters/${encodeURIComponent(currentCharacter.id)}/live2d-part`, {
      method: "POST",
      body: JSON.stringify({
        partId,
...
function isLayerBundleManifestFile(file) {
  return /(^|\/)(layers|manifest|live2d|rig)[^/]*\.json$/i.test(layerBundleFilePath(file)) || /\.json$/i.test(file?.name || "");
}
